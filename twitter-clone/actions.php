<?php
    ...
    if ($_GET['action']=="loginSignup"){
        ...
        if ($_POST['loginActive']=="0"){
            ...
            if (mysqli_num_rows($result) > 0) $error = "That email address has already been registered.";
            else {
                ...
                if(mysqli_query($link,$query)){
                    ...
                    $query = "UPDATE users SET password ='".md5(md5($_SESSION['id']).$_POST['password'])."' WHERE id = ".$_SESSION['id']." LIMIT 1"; //save password as hashed
    ...
    if ($_GET['action'] == 'postTweet'){
        if (!$_POST['tweetContent']) {
            echo "Your tweet cannot be empty!";
        } else if (strlen($_POST['tweetContent']) > 140) {
            echo "Your tweet has to be less than 140 characters.";
        } else {
